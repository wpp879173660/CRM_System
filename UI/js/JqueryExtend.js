    ...
    convertJsonTime: function (time, join) {
        try {
            if (time.length >= 21) {
                ...
                return newTime.getFullYear() + "" + (join.length < 3 && join.length > 0 ? join : "-")
                    + "" + (newTime.getMonth() + 1) + "" + (join.length < 3 && join.length > 0 ? join : "-")
                    + "" + newTime.getDate() + "";
            }
        } catch (e) {
            return time;
...
$.fn.extend({
    ...
    //data:表身内容
    //option:其他操作
    createTableContent: function (thead, data, option) {
        //console.log(option);
        ...
        $.each(data, function (key, value) {
            ...
            $.each(thead, function (key1, value1) {
                ...
                if (value[key1] != undefined) {
                    ...
                    $("<td>").text(value[key1]).appendTo(tbTr);
                }
            });
            ...
            if (option != null) {
                ...
                $.each(option, function (key2, value2) {
                    //console.log(value2);
                    //获取选项提供标签元素
                    var tools = $(value2.toolname).text(value2.text)
                        .click(function () {
    ...
    function paging(index) {
           ...
           var option = {
               ...
               "1": {
                   ...
                   "click": function (row) {
                       if (confirm("确认删除此角色?")) {
                           $.extendAjax("../WebService/_BpWebService.asmx/Delete_Users", { "users": { "UserID": $(row).children().eq(0).text() } }, function (data) {
                               if (data) {
                                   $(row).remove();
                               }
                           });
           ...
           $.extendAjax("../WebService/_BpWebService.asmx/Paging_Users", { "index": index, "size": 4 }, function (data) {
               $(".dataTable")
                   .createTablePaging(thead, JSON.parse(data).PagingData, option,
                       {
                           "pageIndex": index,
    ...
    */
     ...
     *             attrs:所有分页元素单位的属性}
     */
    createTablePaging: function (thead, data, option, pagingData) {
        var table1 = $(this);
        table1.empty();
        $(table1).createTableContent(thead, data, option);
        //分页控制器
        ...
        var toolsDiv=$("<div class='.toolsDiv_'>").appendTo(pagingDiv);
        ...
        $("<div>").text("共" + count + "页 / 第" + index + "页").appendTo(pagingDiv);
